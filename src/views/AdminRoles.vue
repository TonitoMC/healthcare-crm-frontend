<template>
  <div class="flex flex-column h-screen p-3 gap-3">
    <!-- Main Content (fills screen) -->
    <div class="flex flex-column flex-1 gap-3 min-h-0">
      <!-- Top Row: Usuarios + Permisos -->
      <div class="flex gap-3 flex-1 min-h-0">
        <div class="flex-1 min-h-0">
          <UsersTable
            :users="usersView"
            :loading="loading.users"
            :canAdmin="auth.roles.includes('admin')"
            @assign-roles="openAssignRolesDialog"
          />
        </div>
        ...
        <div class="flex-1 min-h-0">
          <PermissionsTable
            ...
            v-model:searchQuery="filters.permissions"
          />
        </div>
      </div>
      ...
      <div class="flex-1 min-h-0">
        <RolesTable
          :roles="filteredRoles"
          :loading="loading.roles"
          v-model:searchQuery="filters.roles"
          :canAdmin="auth.roles.includes('admin')"
          :rolePermissionsMap="rolePermissionsMap"
          :permissions="permissions"
          @view-permissions="openPermissionsDialog"
    ...
    >
      <div class="flex flex-column gap-3">
        <span class="text-sm text-color-secondary">
          Define el nombre y una descripción
        </span>
...
const usersView = computed(() => {
  return users.value.map((u) => ({
    ...u,
    rolesReadable: (u.roles || [])
      .map((r) => r.nombre || r.Name || r.name || "")
      .filter(Boolean),
  }));
});
