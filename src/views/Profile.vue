    ...
    <div class="profile-container">
      ...
      <template #content>
          ...
          <div class="col-12 md:col-6">
            <div class="flex flex-column gap-3">
              ...
              <div class="flex align-items-end gap-2">
                <Password v-model="passwords.current" toggleMask :feedback="false" placeholder="Contraseña actual" class="w-full" />
              </div>
              <div class="flex align-items-end gap-2">
                <Password v-model="passwords.new1" toggleMask :feedback="false" placeholder="Nueva contraseña" class="w-full" />
              </div>
              <div class="flex align-items-end gap-2">
                <Password v-model="passwords.new2" toggleMask :feedback="false" placeholder="Confirmar nueva contraseña" class="w-full" />
              </div>
              <div class="flex justify-content-end gap-2">
                <Button label="Actualizar" icon="pi pi-save" @click="updatePassword" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-content-between">
          <div class="text-color-secondary">Sesión iniciada como <span class="font-medium">{{ auth.username }}</span></div>
...
function updatePassword() {
  if (!passwords.new1 || passwords.new1 !== passwords.new2) {
    toast.add({ severity: "warn", summary: "Las contraseñas no coinciden", life: 2500 });
    return;
  }
  // Placeholder: aquí llamaríamos a un UserService.updatePassword
  toast.add({ severity: "success", summary: "Contraseña actualizada", life: 2000 });
